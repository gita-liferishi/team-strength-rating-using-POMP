...
```{r}
bpm <- read.csv("C:/Users/Suvrorup Mukherjee/Documents/U-M/Winter 25/STATS 531/Final Project/STAT531-Final/data/BPM-new.csv")
...
bpm %>% select(time,Win,Home,`Last 5 Games BPM`,opp_elo,elo, home_attendance) -> red_bpm
...
nba_pomp <- pomp(
    ...
    covar = covariate_table(
        ...
        home = red_bpm$Home,
        attendance_int = red_bpm$home_attendance
    ),
...
```{r}
coef(nba_pomp) <- c(beta1=0.5, beta2 =0.1, sigma=1, home_court_avd=40, alpha=0.05)
...
local_mifs <- future_lapply(future.seed=TRUE,seq_len(ncpu), function(i) {
    nba_pomp |>
        mif2(
            ...
            rw.sd=rw_sd(beta1=0.5, beta2=0.1, home_court_avd=40, alpha=0.05)
        )
})
