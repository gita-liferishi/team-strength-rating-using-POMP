...
arima_22 <- arima(log(bpm_att$elo),order=c(2,1,2), xreg = c(bpm_att$home_attendance))
arima_22
```
...
```{r}
coef(nba_pomp_att) <- c(beta1=0.5, beta2 =0.1, sigma=1, home_court_avd=40, alpha=0.05)
...
Our POMP Models
```{r}
cat("Loglik for POMP Model 1:", max(results$loglik),"\n")
cat("Loglik for POMP Model 2:", max(results2$loglik))
```
