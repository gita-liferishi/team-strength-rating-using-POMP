...
rproc <- Csnippet("
  team_strength += drift * (team_strength + last5_bpm) + rnorm(0, sigma);
")

dmeas <- Csnippet("
  double p = exp(home_court_avd + team_strength - opp_strength) / (1 + exp(home_court_avd + team_strength - opp_strength));
...
nba_pomp <- pomp(
  ...
  paramnames = c("drift", "sigma", "home_court_avd"),
  covar = covariate_table(
    times = bpm$time,
    last5_bpm = bpm$`Last 5 Games BPM`,
    opp_strength = bpm$opp_elo
  ),
  covarnames = c("last5_bpm","opp_strength")
)
```
