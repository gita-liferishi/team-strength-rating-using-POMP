...
trying to add opp_strength as a state
```{r}
rproc2 <- Csnippet("
  team_strength += beta1 *last5_bpm  + rnorm(0, sigma);
  opp_strength += beta2 * opp5_bpm + rnorm(0, sigma);
...
nba_pomp2 <- pomp(
  ...
  paramnames = c("beta1", "beta2","sigma", "home_court_avd"),
  covar = covariate_table(
    times = red_bpm2$time,
...
nba_pomp2 |>
  simulate(
    params=c(beta1=.5,beta2=.5,sigma=1,home_court_avd=40),
    nsim=20,format="data.frame",include.data=TRUE
  ) -> sims2
...
coef(nba_pomp2) <- c(beta1=0.5, beta2=.5,sigma=1, home_court_avd=40)
...
local_mifs2 <- future_lapply(future.seed=TRUE,seq_len(ncpu), function(i) {
  nba_pomp2 |>
    mif2(
      Np=700, Nmif=20,
      cooling.fraction.50=0.5,
      rw.sd=rw_sd(beta1=0.5, home_court_avd=40, beta2=0.5)
    )
})
...
local_mifs_combined2 |>
  traces(pars=c("loglik","beta1","beta2","sigma","home_court_avd")) |>
  melt() |>
  ggplot(aes(x=iteration,y=value,group=.L1,color=factor(.L1)))+
  geom_line()+
  guides(color="none")+
...
running sim on the found best params
```{r}
nba_pomp |>
  simulate(
    params=c(beta1=4.221979,sigma=1,home_court_avd=152.4467,alpha=0.7078109),
    nsim=20,format="data.frame",include.data=TRUE
  ) -> sims
...
sigma_lm <- sigma*sqrt((161-dim(model.matrix(mod))[2])/161) 
cat("Loglik for Linear Regression:",sum(log(dnorm(x = bpm$elo, mean = predict(mod), sd = sigma_lm))))
```

global with two states
```{r}
set.seed(2062379496)
runif_design(
lower=c(beta1=0,home_court_avd=200,beta2=0),
upper=c(beta1=1,home_court_avd=250,beta2=1),
nseq=400
) -> guesses
...
nba_pomp2 |>
  simulate(
    params=c(beta1=0.3001256,beta2=-0.3188534,sigma=1,home_court_avd=100.6434),
    nsim=20,format="data.frame",include.data=TRUE
  ) -> sims2
